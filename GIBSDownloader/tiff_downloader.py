...
class TiffDownloader():
    ...
    @classmethod
    def download_area_tiff(cls, region, date, xml_path, filename, product, res, width=None, height=None, out_frmt="GTiff"):
        if out_frmt == "GTiff":
            extension = 'tif'
        else:
            extension = 'jpeg'
    ...
    def generate_xml(cls, xml_path, name, date):
        xml_base = "<GDAL_WMS><Service name=\"TMS\"><ServerUrl>https://gibs.earthdata.nasa.gov/twms/epsg4326/best/twms.cgi?"
        xml_end = "<TiledGroupName>{name} tileset</TiledGroupName><Change key='${time}'/>{date}</Change></Service></GDAL_WMS>".format(name=name,date=date)
