...
class TiffDownloader():
    ...
    def download_area_tiff(cls, region, date, download_path, xml_path, output, product):
        ...
        xml_path = generate_xml(xml_path, product, date)
        filename = "{}{}_{}.tif".format(output, str(product), date)
        command = "gdal_translate -of GTiff -outsize {w} {h} -projwin {ll} {xml} {f}}".format(w=width, h=height, ll=lon_lat, xml=xml_path, f=filename)
    ...
    def generate_xml(cls, xml_path, product, date):
        ...
        with open('{}{}.xml'.format(xml_path, date), 'w') as xml_file:
